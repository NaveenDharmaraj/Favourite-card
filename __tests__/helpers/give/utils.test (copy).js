...
describe('Testing giving flows util functions', () => {
    ...
    describe('Testing Review screen util functions', () => {
        ...
        // it('Should return donation page data', () => {
        //     const data = {
        //         companiesAccountsData,
        //         donationMatchData,
        //         selectedTaxReceiptProfile,
        //     };
        //     expect(populateDonationReviewPage(reviewGiveData, data, currency, formatMessage, language)).toEqual(donationReviewData);
        // });
        describe('Testing donation flow', () => {
            it('Should not show data if transaction info is empty', () => {
                ...
                expect(populateDonationReviewPage(modifiedData, data, currency, formatMessage, language)).toEqual(undefined);
            });
            describe('Testing account details section', () => {
                it('Should show "To Impact Account" at account details section for adding money to user account', () => {
                    ...
                    expect(populateDonationReviewPage(reviewGiveData, data, currency, formatMessage, language).accountType).toEqual('user');
                    expect(populateDonationReviewPage(reviewGiveData, data, currency, formatMessage, language).mainDisplayText).toEqual('To Impact Account');
                });
                it('Should show Company account at account details section for adding money to company account', () => {
                    ...
                    expect(populateDonationReviewPage(modifiedData, data, currency, formatMessage, language).accountType).toEqual('company');
                    expect(populateDonationReviewPage(modifiedData, data, currency, formatMessage, language).mainDisplayText).toEqual('Nike\'s Impact Account');
                });
            });
            describe('Testing "Frequency" section', () => {
                it('Should show "Add once" at frequency section if "Frequency" is selected as "Add Once"', () => {
                    ...
                    expect(populateDonationReviewPage(modifiedData, data, currency, formatMessage, language).listingData[0]).toEqual(expectedresult);
                });
            ...
            describe('Testing "Tax receipt recipient" section', () => {
                it('Should show only "address one" field at "Tax receipt recipient" section if address two is empty', () => {
                    ...
                    expect(populateDonationReviewPage(reviewGiveData, data, currency, formatMessage, language).listingData[2].name).toEqual('reviewTaxReceipt');
                    expect(populateDonationReviewPage(reviewGiveData, data, currency, formatMessage, language).listingData[2].value[2]).toEqual(' 98567 Altenwerth Spring  street 006 ');
                });
                it('Should show only "address one" field at "Tax receipt recipient" section if address two is empty', () => {
                    const modifiedData = {
                        ...selectedTaxReceiptProfile,
                    ...
                    expect(populateDonationReviewPage(reviewGiveData, data, currency, formatMessage, language).listingData[2].name).toEqual('reviewTaxReceipt');
                    expect(populateDonationReviewPage(reviewGiveData, data, currency, formatMessage, language).listingData[2].value[2]).toEqual(' 98567 Altenwerth Spring   ');
                });
            });
            ...
            describe('Testing "Note to self" section', () => {
                it('Should should show "No note added" if there is no Note to self', () => {
                    const modifiedData = {
                        ...reviewGiveData,
                        noteToSelf: '',
                    };
                ...
                it('Should should show note if there is Note to self', () => {
                    const modifiedData = {
                        ...reviewGiveData,
        ...
        describe('Testing give to flow', () => {
            ...
            //         type: 'give/to/group',
            //     };
            //     expect(populateGiveReviewPage(giveToReviewData, data, currency, formatMessage, language, false).listingData)
            //         .not.toEqual([]);
            // });
            describe('Testing Frequency section', () => {
                it('Should show "Add Once" at "Frequency" section if gift type is selected "Add Once"', () => {
                    const modifiedData = {
                        ...giveToReviewData,
                        giftType: {
                            value: 0,
                ...
                it('Should show "Send monthly 1st of every month" at "Frequency" section if gift type is selected "1st of every month"', () => {
                    const modifiedData = {
                        ...giveToReviewData,
                        giftType: {
                            value: 1,
                ...
                it('Should show "Send monthly 15th of every month" at "Frequency" section if gift type is selected "15th of every month"', () => {
                    const modifiedData = {
                        ...giveToReviewData,
                        giftType: {
                            value: 15,
            ...
            describe('Testing "Info to share with the charity" section', () => {
                it('Should show "Give Anonymously" at "Info to share with the charity" section if Info to share with the charity is not ananymous for give to charity flow', () => {
                    const modifiedData = {
                        ...giveToReviewData,
                        infoToShare: {
                            value: 'anonymous',
                ...
                it('Should show give from information at "Info to share with the charity" section if Info to share with the charity is not ananymous for give to charity flow', () => {
                    const modifiedData = {
                        ...giveToReviewData,
                        infoToShare: {
            ...
            describe('Testing "Info to share with the Giving Group members" section', () => {
            ...
            describe('Testing "Message for Group Admins" section', () => {
                it('Should show "No message added" at "Message for Group Admins" section if there is no message added', () => {
                    const modifiedData = {
                        ...giveToReviewData,
                        noteToCharity: '',
                    };
                ...
                it('Should show message added at "Message for Group Admins" section if there is message added', () => {
                    const modifiedData = {
                        ...giveToReviewData,
                        noteToCharity: 'Test Message',
                    };
        ...
        describe('Testing P2P flow', () => {
            // it('Should show "Give from" section ', () => {
            //     const modifiedData = {
            //         ...P2PReviewData,
            //         giveFrom: {
            //             ...P2PReviewData.giveFrom,
            //             text: 'Mishel David\'s Impact Account: $1,203.00',
            //         },
            //     };
            //     const data = {
            ...
            //         type: 'give/to/friend',
            //     };
            //     expect(populateP2pReviewPage(modifiedData, data, currency, formatMessage, language).listingData[0].name)
            //         .toEqual('reviewGiveFrom');
            //     expect(populateP2pReviewPage(modifiedData, data, currency, formatMessage, language).listingData[0].value)
            //         .toEqual('Mishel David\'s Impact Account: $1,203.00');
            // });
            ...
            it('Should show user name below profile image section for single user if user has profile name', () => {
                const modifiedData = {
                    ...P2PReviewData,
                    recipientImage: 'test.png',
